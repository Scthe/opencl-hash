...
int main(int argc, char **argv) {
  ...
  // Get the devices
  ciErr1 =
      clGetDeviceIDs(cpPlatform, CL_DEVICE_TYPE_GPU, 1, &cdDevice, nullptr);
  ...
  cmDevDst = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE,
  ...
  std::cout << "Kernel length: " << szKernelLength << std::endl;
  checkError(argc, argv, szKernelLength > 0 ? CL_SUCCESS : CL_INVALID_PROGRAM,
             "Error in clCreateProgramWithSource");

  // Create & build the program
  cpProgram = clCreateProgramWithSource(
      cxGPUContext, 1, (const char **)&cSourceCL, &szKernelLength, &ciErr1);
  ...
  for (ull i = 0; i < repeatCnt; i++) {
    ...
    ciErr1 = clEnqueueWriteBuffer(cqCommandQueue,	// cl_command_queue
    command_queue
                                                              cmDevDst,	//
    cl_mem buffer
                                                              CL_FALSE,		//
